...
fossil.locs <-
  readr::read_rds(
    here::here(
      "./vignettes/data/pollen_cleaned/fossil_final_west_metadata_post5500BP.rds"
    )
  ) %>%
...
alley2000 <-
  read.csv(file = "./vignettes/data/external_datasets/gisp2_temp_accum_alley2000.csv", header = TRUE) %>%
...
viau <- read.csv("./vignettes/data/external_datasets/viau.csv") %>%
  dplyr::mutate(time = paleomat::BPtoBCAD(time))

kaufman_standard <-
  read.csv("./vignettes/data/external_datasets/Kaufman 2020_Figure6Data.csv") %>%
...
# Viau and paleomat
viau_paleomat_kaufman %>%
  dplyr::mutate(Alpha = case_when(Series == "This study" ~ 1,
                                  TRUE ~ 0.65)) %>%
...
knitr::kable(paleomat_reconstruction,
             caption = "Final reconstruction output from the loess model for 2999 BC to AD 1996.")
...
ff <-
  ...
  mutate(age = paleomat::BPtoBCAD(age)) %>%
  sf::st_drop_geometry()
