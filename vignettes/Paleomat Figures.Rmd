...
```{r fig1, fig.cap="Figure 1. Modern pollen samples used in this study from the Neotoma Paleoecological Database.", echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all', fig.height = 9.83, fig.width = 16.31}
...
#ggplot2::ggsave("vignettes/figures/Figure1.png", dpi = 600)
...
ggplot2::ggplot(data = Moberg, aes(x = date, y = LF)) +
...
#ggplot2::ggsave("vignettes/figures/Figure5.png", dpi = 600)

```

```{r fig6, fig.cap="Figure 6. Low-frequency July temperature anomaly for the SWUS from this study, the North American July temperature reconstruction (Viau et al., 2006) from 3000 BC to AD 2000, and the terrestrial composite for 30–60 °N (Kaufman et al., 2020), from 3300 BC to AD 1700, in 500-year bins. All series standardized to a mean of 0 and an s of 1 over the period in this graph.", echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all', fig.height = 9.83, fig.width = 16.31}
...
viau <- read.csv(here::here("vignettes/data/external_datasets/viau.csv")) %>%
  dplyr::mutate(time = paleomat::BPtoBCAD(time))

kaufman_standard <-
  read.csv(here::here("vignettes/data/external_datasets/Kaufman 2020_Figure6Data.csv")) %>%
...
viau_paleomat_kaufman <-
  dplyr::bind_rows(temp_anom_standard, viau_standard, kaufman_standard)

viau_paleomat_kaufman$Series <-
  factor(viau_paleomat_kaufman$Series,
         c("Viau et al. 2006", "Kaufman et al. 2020", "This study"))
...
#ggplot2::ggsave("vignettes/figures/Figure6.png", dpi = 600)
...
#ggplot2::ggsave("vignettes/figures/Figure7.png", dpi = 600)
...
sig_pass <-
  ...
  dplyr::mutate(pass = ifelse(pvalue <= 0.10, "Yes", "No")) %>%
...
if (!file.exists(here::here("./vignettes/tables/table1.csv"))) {
  write.csv(table1,
            "./vignettes/tables/table1.csv",
            row.names = FALSE)
}

knitr::kable(table1,
             caption = "Table 1. Metadata for the fossil pollen records (37 sites or 41 datasets) used in this study that have data after 3,550 BC. All data were downloaded from the Neotoma Paleoecological Database (Williams et al., 2018).")
...
fossil_west_post5500 <-
  ...
  dplyr::filter(dataset.id %in% c(250L, 347L, 498L, 517L, 1013L, 1162L, 1660L, 1717L, 1761L, 
...
summary_table_final <-
  ...
  dplyr::mutate(across(c(proportion, pvalue), ~ as.numeric(.x))) %>%
  dplyr::mutate(across(-c(site.name, site.id, dataset.id), ~ round(.x, digits =
                                                                     2)))
...
if (!file.exists(here::here("./vignettes/tables/supp_table_s1.csv"))) {
  write.csv(summary_table_final,
            "./vignettes/tables/supp_table_s1.csv",
            row.names = FALSE)
}

knitr::kable(summary_table_final,
             caption = "Supplemental Table S1. Results of random TF tests and summaries of MAT prediction output for the fossil pollen cores. Random TF test output is shown for the fossil pollen datasets that were included in the random TF test (36 sites or 40 datasets), which was after some samples were removed using squared residual length. An NA denotes that the site was removed after not passing the significance test.")
...
paleomat_reconstruction <- anom_loess4 %>%
  dplyr::mutate(x = as.integer(x)) %>%
  dplyr::filter(x >= -3000 & !duplicated(x)) %>%
  dplyr::mutate(CI = abs(y - ymin)) %>%
  dplyr::select(
    Year = x,
    "Temperature Anomaly (°C)" = y,
...
if (!file.exists(here::here("./vignettes/tables/supp_table_s2.csv"))) {
  write.csv(paleomat_reconstruction,
            "./vignettes/tables/supp_table_s2.csv",
            row.names = FALSE)
}

knitr::kable(paleomat_reconstruction,
             caption = "Supplemental Table S2. Final reconstruction output from the loess model for 2999 BC to AD 1996.")

```

# Supplemental Figures

```{r suppfig1, fig.cap="Supplemental Figure S1. Squared residual length for the fossil assemblages vs age. Red line marks the 95% limit of the calibration set residual lengths (very poorly fitted). We removed all samples that fell above the 95% line.", fig.height = 9.83, fig.width = 16.31, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all'}
...
rlen_results_filtered <-
  readr::read_rds(here::here("./vignettes/data/diagnostics_reconstruction/rlen_full_results2.rds"))$res_lengths %>%
  dplyr::left_join(., ff, by = "sample.id")

ninety5quant <-
  readr::read_rds(here::here("./vignettes/data/diagnostics_reconstruction/rlen_full_results2.rds"))$quantile_95

ggplot2::ggplot(data = rlen_results_filtered, aes(x = age, y = Squared_res_lengths)) +
  geom_point() +
  xlab("Years BC/AD") +
...
#ggplot2::ggsave("vignettes/figures/SuppFigure1.png", dpi = 600)
...
ggplot2::ggplot(data = minDC_df, aes(x = age, y = values)) +
  xlab("Years BC/AD") +
  ylab("Dissimilarity") +
  scale_x_continuous(
    limits = c(-3500, 2100),
  ...
  annotate(
    "label",
    x = Inf,
    y = c(0.1139258, 0.1650981, 0.2171289),
    label = c("1%", "2.5%", "5%"),
    hjust = 0,
    size = 8,
    fill = "white",
...
#ggplot2::ggsave("vignettes/figures/SuppFigure3.png", dpi = 600)
...
#ggplot2::ggsave("vignettes/figures/SuppFigure5.png", dpi = 600)
