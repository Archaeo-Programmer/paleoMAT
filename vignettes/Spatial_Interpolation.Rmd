...
ok <- site.preds.unlisted.anom.nested %>% 
  dplyr::select()
...
ras <- raster(xmn=min(site.locs$long), xmx=max(site.locs$long), ymn=min(site.locs$lat), ymx=max(site.locs$lat))
# Change resolution to 0.1 degree.
res(ras) <- 0.1
...
map_predictions2 <-
  function(site.preds,
    ...
    if (rast.extrap == TRUE) {
      ...
      fit.full <-
        fields::predictSurface(fit_TPS, grid.list, extrap = TRUE)
      fit.full <- raster(fit.full)
      crs(fit.full) <- CRS('+init=EPSG:4326')
      ...
      fit.full <-
        fields::predictSurface(fit_TPS, grid.list, extrap = FALSE)
      fit.full <- raster(fit.full)
      crs(fit.full) <- CRS('+init=EPSG:4326')
...
# Extract just the interpolation of anomaly and remove the error raster.
tavg_preds_temp_new <- purrr::map(tavg_preds_new, 1)

tavg_rstack <- raster::stack(tavg_preds_temp_new)
...
# plot(tavg_rstack[[17:28]], col = colors)

anom_new_tps <- purrr::map(tavg_preds_temp_new, cellStats, mean) %>%
  as.data.frame() %>%
  tidyr::pivot_longer(cols = 1:28) %>%
  dplyr::mutate(name = seq(-950, 1750, 100)) %>%
