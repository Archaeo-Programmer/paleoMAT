...
# Now, we carry out the reconstruction for all fossil pollen sites in the continental United States. 
...
if (!file.exists("./vignettes/data/US_reconst_tavg.rds")) {
  ...
  US_reconst_tavg <- mat_models_temp %>%
    purrr::map(
  ...
  readr::write_rds(US_reconst_tavg, "./vignettes/data/US_reconst_tavg.rds")
  
} else {
  stop(
    "The US reconstruction is already completed. If you would like to re-run the reconstructions, then delete the file from the US directory."
  )
...
if (!file.exists("./vignettes/data/US_reconst_gdd.rds")) {
  mat_models_temp <- purrr::map(
    1:12,
    .f = function(j) {
      rioja::MAT(
  ...
  US_reconst_gdd <- mat_models_temp %>%
    purrr::map(
      .f = function(the.model) {
        # Create the MAT reconstruction.
        predict(
  ...
  readr::write_rds(US_reconst_gdd, "./vignettes/data/US_reconst_gdd.rds")
...
# Simplify tavg
if(!exists("US_reconst_tavg")) {
  US_reconst_tavg <-
    readr::read_rds(here::here("./vignettes/data/US_reconst_tavg.rds"))
}
...
US_reconst_simple_tavg <-
  simplify_reconstruction(reconstruction.data = US_reconst_tavg,
                          fossil.metadata = NAfossil_metadata_counts)


#Simplify GDD
if(!exists("US_reconst_gdd")) {
  US_reconst_gdd <-
    readr::read_rds(here::here("./vignettes/data/US_reconst_gdd.rds"))
}

# Put the reconstructed climate data into a tibble, using the simplify_reconstruction function. The function takes to parameters, the full reconstruction dataset (reconstruction.data) and the fossil dataset (with metadata and counts) that was used initially loaded that has all of the metadata. The function will select only the metadata and ignore the pollen counts. 
US_reconst_simple_gdd <-
  simplify_reconstruction(reconstruction.data = US_reconst_gdd,
                          fossil.metadata = NAfossil_metadata_counts)
