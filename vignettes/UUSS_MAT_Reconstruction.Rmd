...
tavg <- reconst_tavg_final %>%
  unnest(geometry) %>%
  group_by(sample.id, month) %>%
  mutate(col = seq_along(sample.id)) %>% #add a column indicator
  spread(key = col, value = geometry) %>%
  dplyr::rename(long = "1", lat = "2")

# First, for testing and building the map. I will just use the warmest month (i.e., July), and will only pick 2 centuries to do (i.e., 600-700 and 1200-1300).
tavg_600 <- tavg %>% 
     dplyr::filter(month == 5 & month <= 9) %>%
     dplyr::filter(age <= 5000 & month <= 9) %>% 
...
fit <- Tps(cbind(tavg$long, tavg$lat), tavg$value, scale.type="unscaled")
