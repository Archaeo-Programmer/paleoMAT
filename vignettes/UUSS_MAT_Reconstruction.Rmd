...
library(maps)
crs(ok) <- "+proj=lcc +lat_1=49 +lat_2=77 +lat_0=0 +lon_0=-95 +x_0=0 +y_0=0 +ellps=GRS80 +datum=WGS84 +units=m +no_defs"
gplot1 <- as.data.frame(ok, xy = TRUE)
gplot1_transformed <- usmap_transform(gplot1)
...
MDB_sf <- sf::st_as_sf(MDB_full, coords = c("Longitude", "Latitude"), crs = 4326)


ggplot() + 
  geom_raster(data=gplot1_transformed, aes(x=x, y=y, fill=layer), alpha=0.8, na.rm = TRUE) +
