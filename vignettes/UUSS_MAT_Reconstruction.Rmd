...
# Subset the MPCT_final to just the modern pollen.
modern_pollen <- MPCT_final %>%
    dplyr::select(sample.id, pollen) %>%
    sf::st_drop_geometry() %>%
    tidyr::unnest(cols = c(pollen)) %>%
    tibble::column_to_rownames("sample.id")
...
# Subset the MPCT_final to just the modern pollen.
modern_gdd <- MPCT_final %>%
    dplyr::select(sample.id, climate) %>%
    sf::st_drop_geometry() %>%
    tidyr::unnest(cols = c(climate)) %>%
    tibble::column_to_rownames("sample.id") %>% 
    dplyr::select(-contains("ppt"))
...
if (!file.exists("./vignettes/data/US_reconst_gdd_7analog.csv")) {
  
  mat_models_temp_7analog <- purrr::map(1:12,
                                        .f = function(j){
                                          rioja::MAT(y = modern_pollen, 
                                                     x = modern_gdd[,j],
                                                     k = 7, 
  ...
  write.csv(US_reconst_7analog, "./output/US_reconst_gdd_7analog.csv", row.names = FALSE)
