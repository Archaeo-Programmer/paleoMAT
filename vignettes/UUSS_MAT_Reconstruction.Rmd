...
rlen_removal <- purrr::map(fossil_pollen2$pollen, .f = function(x) {rlen <- residLen(spp, env, x, method="cca");
                    names(which(rlen$passive > quantile(rlen$train, probs = 0.90)))}) %>% 
  flatten_chr() %>% 
  as.numeric()
...
# Run for 90%
fossil_input <- dplyr::bind_rows(fos3_90$pollen) %>% 
  dplyr::select(which(colSums(.) > 0))
...
rlgh.mat2 <- predict(object = swap.mat2, newdata = fossil_input, k = 4, bootstrap = T)
...
mat_prediction_1_analogue <- as.data.frame(rlgh.mat2_best_analogue$predictions$model$predicted[1,]) %>%
  tibble::rownames_to_column("sample.id") %>%
  rename(value = 2)
...
write.csv(fos_90, "Tim_All_sig_tests_1best_analog.csv", row.names = FALSE)

# Testing for using just 1 analogue on each site.
...
# tavg_new_90 %>% 
#   filter(date >= -1000) %>% 
# ggplot(., aes(x=date, y=values, group=site.name)) + 
...
tavg_100yrs_new <- tavg_predicted_west_new_90 %>%
  dplyr::select(-model_fit,-plot_fit) %>%
  tidyr::unnest(mean_100yrs) %>%
  bind_rows(., tavg_new_90 %>% filter(site.id %in% c(2260, 1661)) %>% dplyr::select("site.id", "site.name", "elev", "long", "lat","date"= "mid_period" , "value")) %>% #sf::st_drop_geometry() ) %>% 
