...
# for (i in 1:length(unique(tavg_filtered$site.id))){
#   return()
# }
...
tavg_filtered <- tavg %>%
  ...
  dplyr::arrange(age) %>% 
  tidyr::nest(site.data = c(-site.id, -site.name, -elev, -long, -lat)) %>%
  sf::st_as_sf(coords = c("long", "lat"),
                 crs = 4326) %>% 
  dplyr::mutate(mean_100yrs = map(site.data, interpolate_time)) %>% 
