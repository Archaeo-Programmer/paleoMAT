...
# Keep only the area that falls within the continental United States.
west_in_states <-
  sf::st_intersection(all_west, st_union(continental.states$geometry))
west_in_states <- st_union(west_in_states)
#west_in_states_spdf <- sf:::as_Spatial(west_in_states)
west_in_states <- st_cast(west_in_states, "POLYGON")
...
# Update projection to CRS 4326, which is WGS 84.
west_in_states <- sf::st_transform(west_in_states, 4326)
# Need to make the geometry valid for the states shapefile.
west_in_states <- sf::st_make_valid(west_in_states)

# Do an intersection and keep only the sites that are within the western continental United States.
MPCT_metadata_counts_final <- 
  sf::st_intersection(MPCT_metadata_counts, west_in_states)
...
crs(MPCT_metadata_counts_final) <- 4326
...
#readr::write_rds(MPCT_final, here::here("vignettes/data/MPCT_final.rds"))
# MPCT_final <-
