...
tavg_filtered <- tavg %>% 
  dplyr::filter(month == 7 & date >= -1051 & date <= 1800) %>% 
  dplyr::filter(!(dataset.id == 250 & date >= 0))


beef.pasture <- tavg_filtered %>% 
  dplyr::filter(site.id == 1906) %>% 
  dplyr::group_by(age) %>%
...
plot(x,y, xlim = c(-1051, 1800), ylim = c(-4, 8))
lines(fit)
...
plot(eliminated$x, eliminated$y, xlim = c(-1051, 1800), ylim = c(-4, 8))
lines(fit)
...
plot(eliminated$x, eliminated$y, xlim = c(min(fit$x), max(fit$x)))
lines(fit$x, fit$mean)
boxplot(fit$mean)$out
...
beef.pasture.old <- tavg %>% 
  dplyr::filter(dataset.id == 250 & age < 2004)

beef.pasture.new <- tavg %>% 
  dplyr::filter(dataset.id == 25551 & age < 2004)

x1 <- beef.pasture.old$date
y1 <- beef.pasture.old$anom

x2 <- beef.pasture.new$date
y2 <- beef.pasture.new$anom

fit1 <- spline(x1,y1)
fit1a <- smooth.spline(x1,y1)
plot(x1,y1, ylim=c(-5, 5))
lines(fit1)
xgrid1 <- seq(min(fit1$x), max(fit1$x),,2000)
plot(predict(fit1, xgrid1))

#Natural cubic spline versus b-spline.
cr_fit1 <- mgcv::gam(y1 ~ s(x1, bs = 'cr'))
cr_knots1 <- cr_fit1$smooth[[1]]$xp  ## extract knots locations

bs_fit1 <- mgcv::gam(y1 ~ s(x1, bs = 'bs'))
bs_knots1 <- bs_fit1$smooth[[1]]$knots  ## extract knots locations


fit <- spline(x,y)
fita <- smooth.spline(x,y, df = 20)
plot(x,y, ylim=c(-5, 5))
lines(fit)
#Natural cubic spline versus b-spline.
cr_fit <- mgcv::gam(y ~ s(x, bs = 'bs', k = 10))
cr_knots <- cr_fit$smooth[[1]]$xp  ## extract knots locations

bs_fit <- mgcv::gam(y ~ s(x, bs = 'bs'))
...
plot(x, y, col= "grey", main = "Forsythe, Malcolm and Moler cubic spline");
lines(fit)
plot(x, y, col= "grey", main = "Smoothed Spline");
lines(fita)
#Beef pasture old
plot(x1, y1, col= "grey", main = "natural cubic spline");
lines(x1, cr_fit1$linear.predictors, col = 2, lwd = 2)
abline(v = cr_knots1, lty = 2)
plot(x1, y1, col= "grey", main = "B-spline");
lines(x1, bs_fit1$linear.predictors, col = 2, lwd = 2)
abline(v = bs_knots1, lty = 2)
plot(x1, y1, col= "grey", main = "Forsythe, Malcolm and Moler cubic spline");
lines(fit1)
plot(x1, y1, col= "grey", main = "Smoothed Spline");
lines(fit1a)
#Beef pasture new
plot(x2, y2, col= "grey", main = "natural cubic spline");
lines(x2, cr_fit2$linear.predictors, col = 2, lwd = 2)
abline(v = cr_knots1, lty = 2)
plot(x2, y2, col= "grey", main = "B-spline");
lines(x2, bs_fit2$linear.predictors, col = 2, lwd = 2)
abline(v = bs_knots1, lty = 2)
plot(x2, y2, col= "grey", main = "Forsythe, Malcolm and Moler cubic spline");
lines(fit2)
plot(x2, y2, col= "grey", main = "Smoothed Spline");
lines(fit2a)
...
plot(x2,y2, ylim=c(-5, 5))
lines(fit2)
xgrid2 <- seq(min(fit2$x), max(fit2$x),,2000)
plot(predict(fit2, xgrid2))

#Natural cubic spline versus b-spline.
cr_fit2 <- mgcv::gam(y2 ~ s(x2, bs = 'cr'))
cr_knots2 <- cr_fit2$smooth[[1]]$xp  ## extract knots locations

bs_fit2 <- mgcv::gam(y2 ~ s(x2, bs = 'bs'))
...
### Thin Plate Spline Regression and Mapping
