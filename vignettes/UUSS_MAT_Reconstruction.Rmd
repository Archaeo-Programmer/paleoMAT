...
env <- bind_rows(MPCT_proportions$climate) %>% 
  pull(tavg)
...
rlen_removal <- map(fossil_pollen$pollen, .f = function(x) {rlen <- residLen(spp, env, x, method="cca");
                    names(which(rlen$passive > quantile(rlen$train, probs = 0.9)))}) %>% 
  flatten_chr() %>% 
  as.numeric()
...
tavg_new_90 <- fos_90 %>%
  ...
  dplyr::mutate(anom = values - warmest.month)
