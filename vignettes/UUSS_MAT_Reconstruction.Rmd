...
NAfossil_final <- ok <- 
  ...
  dplyr::mutate_at(vars(-sample.id, -geometry), ~ ifelse(. < 0, 0, .)) %>% 
  #mutate_each(funs(replace(. < 0, 0, .)), -geometry) %>% 
...
US_reconst_simple <- NAfossil_metadata_counts %>%
  dplyr::select(dataset.id:pub_year) %>%
  dplyr::mutate(model = "MAT") %>%
  cbind(tibble::tibble(reconst = US_reconst_gdd$fit[, 1],
                       err = US_reconst_gdd$SEP[, 1]))
