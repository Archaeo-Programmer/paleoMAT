...
if (!file.exists("./vignettes/data/reconst_tavg_threshold.rds")) {
  reconst_tavg_threshold <-
    purrr::map(.x = reconst_tavg, .f = apply_threshold)
...
if (!file.exists("./vignettes/data/reconst_gdd_threshold.rds")) {
  reconst_gdd_threshold <-
    purrr::map(.x = reconst_gdd, .f = apply_threshold)
...
# Simplify tavg
if (!exists("reconst_tavg_threshold.rds")) {
  reconst_tavg_threshold <-
...
if (!file.exists("./vignettes/data/reconst_tavg_final.rds")) {
  reconst_tavg_final <-
    ...
    # Remove the rows that have NAs for the reconstructed value (i.e., this indicates that there were too few analogues to do a reconstruction).
    drop_na(value)
...
#Simplify GDD
if (!exists("reconst_gdd_threshold")) {
  reconst_gdd_threshold <-
