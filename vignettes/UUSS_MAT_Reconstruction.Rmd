...
out <- fields::Tps(beef.pasture$date ,beef.pasture$anom, df = 80)
xgrid<- seq(  min(out$x), max(out$x),,40)
...
names(tavg_preds) <- c("BC1000","BC900")

colors <- colorRampPalette(rev(RColorBrewer::brewer.pal(11,"RdBu")))(255)
plot(tavg_preds$site.predictions[[1]], col=colors)

tavg_rbrick <- raster::brick(tavg_preds$site.predictions, varname = "anom")
...
tavg_animated <- raster::animate(tavg_rstack, n =2)
...
bbox <- bbox %>%
  ...
  sf::st_as_sf(crs = 4326) %>%
  sf::st_transform(crs = 4326)
NED <- raster::crop(NED, bbox)

NED.long <- raster::xFromCol(NED)
NED.lat <- raster::yFromRow(NED)[2:962] %>% sort()
NED.elev <- as.matrix(NED)[2:962,]
NED.elev <- t(NED.elev) %>% as.data.frame()
NED.elev <- NED.elev[,order(ncol(NED.elev):1)] %>% as.matrix()
NED.list <- list(NED.long, NED.lat, NED.elev)
names(NED.list) <- c("x", "y", "z")

grid.list<- list(x = NED.list$x, y = NED.list$y)
fit.full<- fields::predictSurface(fit_TPS, grid.list, ZGrid=NED.list, extrap = TRUE)
raster(fit.full)
fit.full.SE <- fields::predictSurfaceSE(fit_TPS, grid.list, ZGrid=NED.list, drop.Z = TRUE, extrap = TRUE)


k <- spatial.tools::modify_raster_margins(NED, extent_delta = c(0, 0, 0, -1))
