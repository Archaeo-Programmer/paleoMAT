...
fossil_final$pollen <- map(fossil_final$pollen, function(x) tibble::column_to_rownames(x, "sample.id"))
...
# Step 1 RLen removal.
rlen_removal <- map(fossil_pollen2$pollen, .f = function(x) {rlen <- residLen(spp, env, x, method="cca");
                    names(which(rlen$passive > quantile(rlen$train, probs = 0.90)))}) %>% 
  flatten_chr() %>% 
  as.numeric()


rlen_results <- map(fossil_pollen2$pollen, .f = function(x) {rlen <- residLen(spp, env, x, method="cca");
stack(rlen$passive)}) %>% 
  bind_rows() %>% 
...
fos <- fossil_pollen
fos$pollen <- map(fos$pollen, ~ .x %>% tibble::rownames_to_column("sample.id") %>% filter(!sample.id %in% rlen_removal) %>% tibble::column_to_rownames("sample.id"))
