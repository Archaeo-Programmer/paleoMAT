...
writeRaster(tavg_idw_brick, filename="./output/paleomat-rasters.tif", 
...
anom_new <- purrr::map(tavg_idw_stack, cellStats, mean) %>% 
  as.data.frame() %>% 
  tidyr::pivot_longer(cols = 1:28) %>% 
  dplyr::mutate(name = seq(-950, 1750, 100)) %>% 
  dplyr::rename(year = name, anom = value)

write.csv(anom_new, "./output/paleomat-temp-anomaly-final.csv", row.names = FALSE)
