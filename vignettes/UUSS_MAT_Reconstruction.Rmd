...
# Apply a threshold to the reconstruction, so that if an analgoue is too dissimilar that it will not be used in the reconstruction. Then, based on the remaining analgoues, the final reconstruction and associated error will be re-calculated. 
...
reconst_simple_tavg <-
  simplify_reconstruction(reconstruction.data = reconst_tavg,
                          fossil.metadata = fossil_metadata_counts_final)


#Simplify GDD
if(!exists("reconst_gdd")) {
  reconst_gdd <-
...
# Put the reconstructed climate data into a tibble, using the simplify_reconstruction function. The function takes to parameters, the full reconstruction dataset (reconstruction.data) and the fossil dataset (with metadata and counts) that was used initially loaded that has all of the metadata. The function will select only the metadata and ignore the pollen counts. 
reconst_simple_gdd <-
  simplify_reconstruction(reconstruction.data = reconst_gdd,
                          fossil.metadata = fossil_metadata_counts_final)
