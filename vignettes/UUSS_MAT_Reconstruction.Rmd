...
#readr::write_rds(fossil_metadata_counts_final2, here::here("vignettes/data/fossil_final_west_metadata.rds"))
...
# 0.05
# 11
sum(as.numeric(flatten(sig_results2))<=0.2)
# 0.1
# 19
...
only_90 <- stack(unlist(sig_results2)) %>%
  ...
  mutate(sig_results2 = case_when(values <= 0.2 ~ TRUE,
                                                       TRUE ~ FALSE)) %>% 
  filter(sig_results2 == TRUE) %>% 
  pull(ind) %>% 
  as.numeric(levels(.))[.]
...
mat_prediction <- as.data.frame(rlgh.mat2$predictions$model$predicted[2,]) %>%
  tibble::rownames_to_column("sample.id") %>%
  rename(value = 2)

mat_prediction_4 <- as.data.frame(rlgh.mat2$predictions$model$predicted[4,]) %>%
  mutate(sample.id = fossil_samples) %>% 
  rename(value = 1) %>% 
  mutate(bootstrap = rlgh.mat2[["predictions"]][["bootstrap"]][["predicted"]][,4],
         error = rlgh.mat2[["predictions"]][["sample.errors"]][["rmsep"]][,4])
...
tavg_100yrs_new <- tavg_predicted_west_new_90 %>%
  dplyr::select(-model_fit,-plot_fit) %>%
  tidyr::unnest(mean_100yrs) %>%
  bind_rows(., tavg_new_90 %>% filter(site.id %in% c(1661)) %>% dplyr::select("site.id", "site.name", "elev", "long", "lat","date"= "mid_period" , "value")) %>% #sf::st_drop_geometry() ) %>% 
...
site.preds.unlisted <-
  cbind(site.preds.unlisted, modern = modern_temperature$modern)
...
write.csv(site.preds.unlisted.anom, "./vignettes/data/output/Anom_All3Steps.csv", row.names = FALSE)
