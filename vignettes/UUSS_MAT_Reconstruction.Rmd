...
fossil_final_west_metadata_post5500BP <- fossil_metadata_counts_final2 %>% 
  filter(age <= 5500)

readr::write_rds(fossil_final_west_metadata_post5500BP, here::here("vignettes/data/fossil_final_west_metadata.rds"))
...
# Testing
```{r testing, cache=TRUE, results='asis', echo=FALSE, warning=FALSE}
...
# env <- climate_full %>%
#   group_by(sample.id) %>% 
#   mutate(tavg = mean(tavg)) %>% 
#   select(-month) %>% 
#   distinct() %>% 
#   pull(tavg)


fossil_final$pollen <- purrr::map(fossil_final$pollen, function(x) tibble::column_to_rownames(x, "sample.id"))
...
fossil_pollen2 <- fossil_pollen %>% 
  mutate(rows = map_dbl(pollen, nrow)) %>% 
  filter(!rows %in% c(0, 1))



# COMBO #2 - RUN ALL 3 STEPS (i.e., RLen removal, RandomTF removal, and no analgoue removal)

# Step 1 RLen removal.
rlen_removal <- purrr::map(fossil_pollen2$pollen, .f = function(x) {rlen <- residLen(spp, env, x, method="cca");
                    names(which(rlen$passive > quantile(rlen$train, probs = 0.95)))}) %>% 
  flatten_chr() %>% 
  as.numeric()


rlen_results <- purrr::map(fossil_pollen2$pollen, .f = function(x) {rlen <- residLen(spp, env, x, method="cca");
stack(rlen$passive)}) %>% 
  bind_rows() %>% 
  rename("Squared_res_lengths" = 1, "sample.id" = 2)

rlen_ninety5quant <- purrr::map(fossil_pollen2$pollen, .f = function(x) {rlen <- residLen(spp, env, x, method="cca"); as.numeric(quantile(rlen$train, probs = 0.95))}) %>% 
  first()
...
rlen_full_results2 <- structure(list(res_lengths = rlen_full_results1,
                         quantile_95 = rlen_ninety5quant),
                    class = "list")

readr::write_rds(rlen_full_results2, here::here("vignettes/data/rlen_full_results2.rds"))
  

# depths <- fossil_metadata_counts_final %>%
#   #filter(!is.na(age) & !sample.id %in% rlen_removal) %>%
#   filter(!is.na(age)) %>%
#   pull(depth)
# 
# rlen<-residLen(spp, env, bind_rows(fossil_pollen$pollen), method="cca")
...
fos <- fossil_pollen
fos$pollen <- purrr::map(fos$pollen, ~ .x %>% tibble::rownames_to_column("sample.id") %>% filter(!sample.id %in% rlen_removal) %>% tibble::column_to_rownames("sample.id"))
...
# sig_results1 <-
#   map2(fos$pollen, fos$dataset.id, function(x, y) {
#     output <- randomTF(
#       spp = spp * 100,
...
#   })
# 
# sum(as.numeric(flatten(sig_results1))<=0.05)
# # 0.05
# # 11
# sum(as.numeric(flatten(sig_results1))<=0.1)
# # 0.1
# # 19



# Step 2 - Random TF testing remove sites where tJuly can't explain most at 90%. 
sig_results2 <-
...
sig_results2_full <- map2(fos2$pollen, fos2$dataset.id, function(x, y) {
  output <- randomTF(
    spp = spp,
    env = data.frame(tjul = env),
    fos = x,
...
sig_results2_final <- left_join(sig_results2_proportions, sig_results2_pvalues, by = "dataset.id") %>% 
  dplyr::select(dataset.id, proportion, pvalue) %>% 
  mutate(dataset.id = as.numeric(levels(dataset.id)))
  
readr::write_rds(sig_results2_final, here::here("vignettes/data/rlen_full_results2.rds"))
...
sum(as.numeric(flatten(sig_results2))<=0.05)
# 0.05
# 11
sum(as.numeric(flatten(sig_results2))<=0.1)
# 0.1
# 19

# ok %>% 
#   filter(values < 0.05) %>% 
#   mutate(test = values <= 0.05 & values2 <= 0.05) %>% view


# All_sig_tests <- read.csv("All_sig_tests.csv", header = T, row.names = NULL)[,1:10]
# names(All_sig_tests) <- c(names(All_sig_tests)[-1], "drop")
# All_sig_tests <- All_sig_tests[, -ncol(All_sig_tests)]
  


# only_95 <- All_sig_tests %>%
#   select(dataset.id, sig_results2) %>% 
#   mutate(sig_results2 = case_when(sig_results2 <= 0.051 ~ TRUE,
#                                                        sig_results2 > 0.05 & sig_results2 <= 0.1 ~ FALSE,
#                                                        TRUE ~ FALSE)) %>% 
#   filter(sig_results2 == TRUE) %>% 
#   pull(dataset.id)

# only_95 <- stack(unlist(sig_results2)) %>%
#   mutate(sig_results2 = case_when(values <= 0.051 ~ TRUE,
#                                                        values > 0.05 & values <= 0.1 ~ FALSE,
#                                                        TRUE ~ FALSE)) %>% 
#   filter(sig_results2 == TRUE) %>% 
#   pull(ind) %>% 
#   as.numeric(levels(.))[.]
# 
# fos2_95 <- fos %>% 
#   filter(dataset.id %in% only_95)


# only_90 <- All_sig_tests %>%
#   select(dataset.id, sig_results2) %>% 
#   mutate(sig_results2 = case_when(sig_results2 <= 0.1 ~ TRUE,
#                                                        TRUE ~ FALSE)) %>% 
#   filter(sig_results2 == TRUE) %>% 
#   pull(dataset.id)

only_90 <- stack(unlist(sig_results2)) %>%
  #mutate(values = round(values, digits = 1)) %>% 
  mutate(sig_results2 = case_when(values <= 0.1 ~ TRUE,
                                                       TRUE ~ FALSE)) %>% 
  filter(sig_results2 == TRUE) %>% 
  pull(ind) %>% 
  as.numeric(levels(.))[.]
...
fos3_90 <- fos %>% 
  filter(dataset.id %in% only_90)
  

full_fos <- randomTF(
      spp = spp,
      env = data.frame(tjul = env),
      fos = bind_rows(fos3_90$pollen),
      n = 10000,
      fun = MAT,
      col = 1
    )
...
#sfossil_samples <- unlist(map(fos3_90$pollen, function(x) row.names(x)))

# Run for 90%
fossil_input <- do.call(rbind, unname(fos3_90$pollen)) %>% 
  #dplyr::bind_rows(fos3_90$pollen) %>% 
  dplyr::select(which(colSums(.) > 0))
...
readr::write_rds(swap.mat2, here::here("vignettes/data/mat_calibration.rds"))
# getK(swap.mat2)
...
#rlgh.mat2b1 <- predict(object = swap.mat2, newdata = fossil_input, k = 1, bootstrap = T)
...
# 
# mat_prediction_1_analogue <- as.data.frame(rlgh.mat2b$predictions$model$predicted[1,]) %>%
#   mutate(sample.id = fossil_samples) %>% 
#   rename(value = 1) %>% 
#   mutate(bootstrap = rlgh.mat2b[["predictions"]][["bootstrap"]][["predicted"]][,1],
#          error = rlgh.mat2b[["predictions"]][["sample.errors"]][["rmsep"]][,1])

# rlgh.mat2_copy <- rlgh.mat2
# replace_names <- data.frame(sample.id = colnames(rlgh.mat2$predictions$model$predicted)) %>% 
#   mutate(sample.id = as.integer(sample.id)) %>% 
#   left_join(., fossil_metadata_counts_final %>% select(sample.id, age), by = "sample.id") %>% 
#   pull(age)
# colnames(rlgh.mat2_copy$predictions$model$predicted) <- replace_names
# 
# rlgh.mat2_copy$predictions$model$predicted <- rlgh.mat2_copy$predictions$model$predicted[ , order(colnames(rlgh.mat2_copy$predictions$model$predicted))]
# 
# reconPlot(rlgh.mat2_copy, replace_names, use.labels = F, display.error = "bars",
#           xlab = "Age", ylab = "Temperature")
...
readr::write_rds(minDC_full_results, here::here("vignettes/data/minDC_full_results.rds"))
...
write.csv(fos_90, "Tim_All_sig_tests_1best_analog_boostrap.csv", row.names = FALSE)
...
full_fos <- randomTF(
      spp = pollen_input,
      env = data.frame(tjul = climate_input),
      fos = fossil_input,
      n = 100,
      fun = MAT,
      col = 1
...
# COMBO #1 - FULL RECONSTRUCTION - ALL SITES.
...
rlgh.mat2 <- predict(object = swap.mat2, newdata = fossil_input, k = 5)


mat_prediction <- as.data.frame(rlgh.mat2$predictions$model$predicted[5,]) %>%
  tibble::rownames_to_column("sample.id") %>%
  rename(value = 2)
...
# Testing OLD
```{r testing_OLD, cache=TRUE, results='asis', echo=FALSE, warning=FALSE}
# sig_results3 <- map2(fos$pollen, fos$dataset.id, function(x,y) {output <- randomTF(spp = spp*100, env = data.frame(tjul = env),
#    fos = x*100, n = 10000, fun = MAT, col = 2); sig_out <- output$sig; names(sig_out) <- y; return(sig_out)})
# 
# sum(as.numeric(flatten(sig_results3))<=0.05)
# # 0.05
# # 10
# sum(as.numeric(flatten(sig_results3))<=0.1)
# # 0.1
# # 19
# 
# sig_results4 <- map2(fos$pollen, fos$dataset.id, function(x,y) {output <- randomTF(spp = spp, env = data.frame(tjul = env),
#    fos = x, n = 10000, fun = MAT, col = 2); sig_out <- output$sig; names(sig_out) <- y; return(sig_out)})
# 
# sum(as.numeric(flatten(sig_results4))<=0.05)
# # 0.05
# # 11
# sum(as.numeric(flatten(sig_results4))<=0.1)
# # 0.1
# # 19

# fos_meta <- fossil_metadata_counts_final %>% 
#   select(dataset.id, site.id, site.name) %>% 
#   distinct()
# 
# All_temp <- cbind(sig_results1 = unlist(flatten(sig_results1)), sig_results2 = unlist(flatten(sig_results2)), sig_results3 = unlist(flatten(sig_results3)),sig_results4 = unlist(flatten(sig_results4))) %>% 
#   data.frame()
# 
# All <- All_temp %>% 
#   mutate("95%" = ifelse(rowSums(All_temp <= 0.05) > 0, TRUE, FALSE),
#          "90%" = ifelse(rowSums(All_temp <= 0.1) > 0, TRUE, FALSE)) %>% 
#   tibble::rownames_to_column("dataset.id") %>% 
#   mutate(dataset.id = as.integer(dataset.id)) %>% 
#   left_join(., fos_meta, by = "dataset.id")
# 
# write.csv(All, "All_sig_tests.csv", row.names = FALSE)


rlghr <- randomTF(spp = spp*100, env = data.frame(tjul = env),
   fos = fos$pollen[[1]]*100, n = 100, fun = MAT, col = 1)



rlghr <- randomTF(spp = modern_p_prop*100, env = data.frame(tjul = climate_input[,"tjul"]),
   fos = fossil_p_prop2*100, n = 10000, fun = MAT, col = 1)
...
plot(rlen) # distribution of modern and fossil residual lengths
plot(1:nrow(fos$pollen[[1]]), rlen$passive, ylab="Squared residual length", xlab="Depth")
abline(h=quantile(rlen$train, probs = c(0.9,0.95)), col=c("orange", "red"))
...
mat_reconst <- run_tf(modern_p,
       climate = as.data.frame(climate_input),
       func = MAT, col = 1, k = 5)

mat_sig <- run_tf(pollen = modern_p, 
                  climate = climate_input, 
                  func = MAT, col = 1, k = 5)

mat_sig2 <- run_tf(pollen = Pollen[,2:ncol(Pollen)], 
                  climate = Climate[,2:13], 
                  func = MAT, col = 1, k = 10)

mat_sig3 <- run_tf(pollen = SWAP$spec, 
                  climate = SWAP$pH, 
                  func = MAT, col = 1, k = 10)

temp <- rowMeans(climate_input[,c(5:8)])
fossil_p_prop2 <- fossil_p_prop[-c(12:13),]

rlghr <- randomTF(spp = modern_p_prop*100, env = data.frame(tjul = climate_input[,"tjul"]),
   fos = fossil_p_prop2*100, n = 10000, fun = MAT, col = 1)

rlghr <- randomTF(spp = sqrt(modern_p_prop), env = data.frame(pH = climate_input),
   fos = sqrt(fossil_p_prop), n = 99, fun = MAT, col = 1)

rlghr <- randomTF(spp = sqrt(SWAP$spec), env = data.frame(pH = SWAP$pH),
   fos = sqrt(RLGH$spec), n = 99, fun = WA, col = 1)


spp<-modern_p_prop*100
env<-climate_input[,"tjun"] %>% pull(tjun)
fos<-fossil_p_prop*100
depths<-as.numeric(rownames(fossil_p_prop))

# For some reason this fails consistently when we try to use the weighted MAT (col=2).
modern_significance <- palaeoSig::randomTF(spp = 
                      MPCT_proportions %>%
                    ...
                    col = 1,
                    k = 150) %$%
  tibble::tibble("variable" = colnames(MPCT_proportions$climate[[1]]),
                 "% Explained" = EX*100,
                 "p-value" = sig)
...
# pollen_reduced <- dplyr::bind_rows(MPCT_final$pollen)
# pollen_reduced <- pollen_reduced[, which(colSums(pollen_reduced) != 0)]
# 
# july_after <- 
#       rioja::MAT(
#         y = pollen_reduced,
    ...
    purrr::map(
      .f = function(the.model) {
        ...
        predict(
          ...
          k = which.min(as.data.frame(rioja::performance(july)[["object"]])$RMSE[1:15]),
          sse = TRUE
        )
      }
    )

#which.min(as.data.frame(rioja::performance(july)[["object"]])$RMSE[1:15])

# new_output_recont <-
#   predict(
#     object = july_after,
#     newdata = dplyr::bind_rows(fos3_90$pollen),
#     k = 3,
#     sse = TRUE
#   )

july_modern_calibration <- dplyr::bind_rows(MPCT_proportions$pollen) %>% 
  select(which(colSums(.) > 0))

# Run for 90%
foss_pct_p <- dplyr::bind_rows(fos3_90$pollen) %>% 
  select(which(colSums(.) > 0))

keep_cols <- colnames(july_modern_calibration[colSums(july_modern_calibration) > 0])
keep_cols <- keep_cols[keep_cols %in% colnames(foss_pct_p)]

pollen_input <- july_modern_calibration[, keep_cols]
...
#july_prop_quantiles <- quantile(july_prop[["dist.n"]], probs = c(.01, .02, .05, .10, .20), na.rm = TRUE)
#         1%         2%         5%        10%        20% 
# 0.03271691 0.03958231 0.05214386 0.06720974 0.09107295 
...
#   predict(
#     object = july_after,
#     newdata = dplyr::bind_rows(fos2_95$pollen),
#     k = 3,
#     sse = TRUE
#   )

# Run for 95%
# foss_pct_p <- dplyr::bind_rows(fos2_95$pollen) %>% 
#   select(which(colSums(.) > 0))
# 
# keep_cols <- colnames(july_modern_calibration[colSums(july_modern_calibration) > 0])
# keep_cols <- keep_cols[keep_cols %in% colnames(foss_pct_p)]
...
# age<-depth
# 
# plot(depths, pred$dist.n[,5], ylab="Squared chord distance", xlab="Depth")
# #goodpoorbad<-quantile(paldist(spp), prob=c(0.05, 0.1))
...
climate_input <- bind_rows(MPCT_climate_final$climate) %>% 
  ...
  dplyr::select(month = labels, tavg) %>% 
...
readr::write_rds(site.preds.unlisted.anom, here::here("vignettes/data/final_paleomat_dataset.rds"))
