...
x <- beef.pasture.old$age
y <- beef.pasture.old$anom

x1 <- beef.pasture.new$age
y1 <- beef.pasture.new$anom

fit <- spline(eliminated$age, eliminated$anom)
plot(x,y, ylim=c(-5, 5))
lines(fit)
xgrid <- seq(min(fit$x), max(fit$x),,2000)
plot(predict(fit, xgrid))
...
out <- Tps(x,y)
xgrid<- seq(  min(out$x), max(out$x),,5950)
