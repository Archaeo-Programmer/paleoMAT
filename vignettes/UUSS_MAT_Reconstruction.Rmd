...
ok <- reconst_tavg_final %>% 
  dplyr::filter(age < -12 & month == 7)
  


beef.pasture <- tavg %>% 
  dplyr::filter(site.id == 246 & age < 2004) %>% 
  dplyr::group_by(age) %>% 
...
ggbetweenstats(data = beef.pasture,
...
ggbetweenstats(eliminated, site.id, anom, outlier.tagging = TRUE)
...
fita <- smooth.spline(x,y, df = length(x)*0.10)
fit1a <- smooth.spline(x1,y1, df = length(x1)*0.10)
fit2a <- smooth.spline(x2,y2, spar = 0.1015)
par(mfrow = c(1,3))
