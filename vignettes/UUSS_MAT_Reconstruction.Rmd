...
july_prop <- 
      rioja::MAT(
        y = dplyr::bind_rows(MPCT_proportions$pollen),
...
modern_p2 <- select(modern_p, which(colSums(modern_p) > 0))
...
row.names(fossil_p) <- fossil_final_reduced$sample.id
fossil_p2 <- select(fossil_p, which(colSums(fossil_p) > 0))
...
newoutput <- left_join(mat_prediction_cleaned %>% mutate(sample.id = as.integer(sample.id)), fossil_metadata_counts_final %>% select(dataset.id:pub_year), by = "sample.id") %>% 
    dplyr::left_join(updated_age_models, by = c("site.id", "dataset.id", "sample.id")) %>%
    dplyr::mutate(age = dplyr::coalesce(age.y, age.x)) %>%
    dplyr::select(-age.y, -age.x) %>%
    dplyr::mutate(age = round(age))
...
sample_filter <- c(41285, 41284, 34305, 31510, 31511, 21844, 21845, 21846, 21847, 21848,
                   ...
                   61609, 61610, 61636, 61637, 62179, 62180, 62181, 62183, 62184,
...
# 
tavg_predicted_west_new <- tavg_new %>%
  filter(!sample.id %in% sample_filter) %>% 
  filter(site.id != 9830) %>% 
  dplyr::group_by(site.id, site.name, elev, long, lat) %>%
  tidyr::nest() %>%
...
# Next, the site data is nested together for each 100 years so that the predictions and maps are done for each 100 years.
tavg_100yrs_new <- tavg_predicted_west_new %>%
  dplyr::select(-model_fit,-plot_fit) %>%
  tidyr::unnest(mean_100yrs) %>%
  bind_rows(., individual_samples_filtered %>% ungroup %>% select("site.id", "site.name", "elev", "long", "lat", "mid_period" ,"value") %>% rename(date = "mid_period")) %>% 
