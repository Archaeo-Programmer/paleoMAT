...
fit2 <- Tps(cbind(tavg_1200$long, tavg_1200$lat), tavg_1200$value, lon.lat = TRUE)
surface(fit2, nx=100, ny=100)
...
UUSS_prism <- raster::mask(prism, polygon)
...
surf.1.pred <- predict.Krig(fit2, UUSS_prism_df$coords)

surf.1.pred.m <- matrix(surf.1.pred,
 length(UUSS_prism_df$coords[,1]),
 length(UUSS_prism_df$coords[,2]),
 byrow=F)
...
plot(UUSS_prism_df$coords, pch=20, asp=1, cex=.6,
