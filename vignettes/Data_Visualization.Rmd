...
# GAM
model_gam <-
  mgcv::gam(anom ~ s(date, bs = "cr", k = 50),
            data = site.preds.unlisted.anom,
            family = gaussian())
plot(model_gam)
gam_knots <-
  model_gam$smooth[[1]]$xp  ## extract knots locations
...
predict.points <- unique(site.preds.unlisted.anom$mid_period) %>% 
  sort()
...
# Animate
plot_tps <- ggplot(data = anom2,
                   aes(x = name,
                       y = value)) +
  geom_line(colour = "red", size = 1.0, aes(y = value)) +
...
anim_save(animation = anim2, filename ="/Users/andrew/Dropbox/WSU/SKOPEII/Figures/anomaly_USGS.gif")
