...
UUSS_modern <- sf::st_intersection(MPCT_final, bb)
UUSS_modern2 <- UUSS_modern[!duplicated(UUSS_modern$geometry),]
UUSS_modern2 <- UUSS_modern2 %>% dplyr::select(-pollen)
UUSS_modern2 <- as.data.frame(UUSS_modern2 %>% unnest_longer(climate))
UUSS_modern2$climate <- UUSS_modern2$climate %>% dplyr::select(-dplyr::contains(c("ppt", "gdd")))
UUSS_modern2$climate <- UUSS_modern2$climate %>% dplyr::select(-c(1:4, 10:12))
...
July_anom <- UUSS_100yrAvgs_tavg_July %>% dplyr::mutate(anom = value_mean - warmest.month)

UUSS_100yrAvgs_tavg_anom <- ggplot(data=(dplyr::filter(July_anom[1:50, ])), aes(x=midP, y=anom, label = value_n, label2 = value_unique_sites)) + 
  #scale_color_gradient(low="blue", high="red") +
  # geom_ribbon(aes(x=midP, ymin = value_mean - error_mean,
  #                 ymax = value_mean + error_mean,
  #                 alpha = 0.2),
