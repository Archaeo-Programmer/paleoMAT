...
tavg_MDB_list <-
  mapply(c, tavg_recon_list, MDB_sf_list, SIMPLIFY = FALSE)

maize_anomaly_plots <-
  purrr::map(
    tavg_MDB_list,
    .f = function(x) {
      map_anomaly_maize(x, usa = usa, us.cities = us.cities)
    }
  )


maize_anomaly_plots_arranged <- gridExtra::marrangeGrob(maize_anomaly_plots, nrow=2, ncol=1, top=quote(names(maize_anomaly_plots)[g]), gp=gpar(fontsize=15,font=8))

# Save as one pdf. Use scale here in order for the multi-plots to fit on each page.
ggsave("/Users/andrew/Dropbox/WSU/SKOPEII/Figures/maize_anomaly_plots_arranged.pdf", maize_anomaly_plots_arranged, scale = 1.5, 
       width = 210, 
       height = 297, 
       units = "mm")
