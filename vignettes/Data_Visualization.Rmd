...
UUSS_100yrAvgs_tavg_GS <- reconst_tavg_final %>% 
     ...
     dplyr::summarise_at(c("value", "error", "elev"), list(mean, n = n(), unique_sites = dplyr::n_distinct(site.id), unique_datasets = dplyr::n_distinct(dataset.id))) %>%
...
UUSS_100yrAvgs_tavg_July <- reconst_tavg_final %>% 
     ...
     dplyr::summarise_at(c("value", "error", "elev"), dplyr::funs(mean, n = dplyr::n(), unique_sites = dplyr::n_distinct(site.id), unique_datasets = dplyr::n_distinct(dataset.id))) %>%
     dplyr::arrange(as.numeric(period)) %>% 
     dplyr::select(-c(error_n, elev_n, error_unique_sites, error_unique_datasets, elev_unique_sites, elev_unique_datasets)) %>% 
...
# Limit modern sites to just the Southwest for mean temperature.
...
# Get prism grid for July, then subset to the tmax and tmin raster layers. 
ok <- prism_7$normal
ok <- ok[2:3]
# Create a raster stack for tmax and tmin.
ok2 <- raster::stack(ok[[1]], ok[[2]])
# Change layer names to tmax and tmin.
names(ok2) <- c('tmax', 'tmin')
# Sum the tmax and tmin raster layers in order to get average temperature (tmax + tmin / 2).
ok3 <- calc(ok2, fun = sum)
# Then divide every cell by 2 to get the mean temperature. Finally, PRISM extractions are all multiplied by 10 in order to store them as integers. Therefore, to get to deg_C for temperature, divide by 10.
fun_conv <- function(x) { (x / 2)/10 }
July_mean <- calc(ok3, fun_conv)

# Do intersection to get just the spatial extent of the fossil pollen sites.
UUSS_prism <- raster::mask(July_mean, fossilpolygon)

#Get mean for all pixels in the defined study area.
UUSS_prism_mean <- raster::cellStats(UUSS_prism, mean, na.rm = TRUE)
...
# Get mean temperature for prism data for the July and for the growing season. 
...
July_anom <- UUSS_100yrAvgs_tavg_July %>% 
  dplyr::mutate(anom =  value_mean - UUSS_prism_mean)

UUSS_100yrAvgs_tavg_anom <- ggplot(data=(dplyr::filter(July_anom[44:60, ])), aes(x=midP, y=anom, label = value_n, label2 = value_unique_sites)) + 
  geom_line(colour="red", size = 3.0) +
  xlab("Year BC/AD") +
  ylab("Temperature Anomaly") +
  scale_x_continuous(breaks=seq(300,2100,200), minor_breaks = seq(400, 2000, 200)) +
  scale_y_continuous(breaks=seq(-10,4,1), limits = c(-10, 4)) +
  theme_bw() + 
  theme(panel.border = element_blank(), panel.grid.major = element_blank(), axis.text=element_text(size=18, colour = "black", family = "Helvetica"), axis.title.y = element_text(size = 28, family = "Helvetica", margin = margin(t = 10, r = 20, b = 10, l = 10)), axis.title.x = element_text(size = 28, family = "Helvetica", margin = margin(t = 20, r = 10, b = 10, l = 10)),
        panel.grid.minor = element_line(colour = "light grey"), axis.line = element_line(colour = "black"), legend.text = element_text(size = 18, family = "Helvetica"), legend.title = element_text(size = 22, family = "Helvetica")) +
  labs(col = "Temperature (°C)")
...
# Save the plot for GS.
ggsave(UUSS_100yrAvgs_tavg_anom, file = "UUSS_100yrAvgs_tavg_anom.pdf", path = "vignettes/figures", units="in", width=21, height=13)
