...
gp1 <- UUSS_Puebloperiods %>% ggplot() +
  ...
  gp2 <- gp1 %+% scale_y_continuous(name = expression("Temperature ("~degree~"C)"), sec.axis = sec_axis(~ .* 3000  , name = "Elevation (meters)"), limits = c(-1, 2))
  
  

# Plot the all monthly temperatures lumped together. 
UUSS_Puebloperiods_plot <- ggplot(data=(dplyr::filter(UUSS_Puebloperiods))) +
      geom_line(aes(x=date_midpoint, y=reconst_mean, colour = "red")) +
...
UUSS_Puebloperiods_Elev_plot <- ggplot(data=(dplyr::filter(UUSS_Puebloperiods_elevation)), aes(x=date_midpoint, y=mean, color = mean)) +
    ...
    # Save the plot for one month.
    ggsave(UUSS_Puebloperiods_Elev_plot, file = "UUSS_Puebloperiods_Elev_plot.tiff", path = "./output/UUSS/")
    
    # Create html files that have the interactive plots from plotly.
    UUSS_Puebloperiods_Elev_plot_ggplotly <- ggplotly(UUSS_Puebloperiods_Elev_plot)
    f<-"./output/UUSS/plotly_htmls/UUSS_Puebloperiods_Elev_plot_ggplotly.html"
     htmlwidgets::saveWidget(UUSS_Puebloperiods_Elev_plot_ggplotly, file.path(normalizePath(dirname(f)),basename(f)))
