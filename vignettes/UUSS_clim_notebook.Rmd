...
packages <- c("tidyverse", "purrrlyr", "reshape2", "plyr", # For tidy data
              "foreach", "doParallel", # Packages for parallel processeing
              "rebus", # For other useful functions
              "rgdal",
              "knitr", # For rmarkdown
...
coreTops <- readr::read_csv("./data/surface_coretop_Minmeta.csv") %>% 
  dplyr::distinct()
...
NAfossil_metadata_counts <-  dplyr::anti_join(NAfossil_metadata_counts, 
                                              coreTops, 
                                              by = c("dataset.id", "site.id", "sample.id"))
...
black.point <- leaflet::makeIcon(iconUrl = "https://upload.wikimedia.org/wikipedia/commons/thumb/1/11/BlackDot.svg/2000px-BlackDot.svg.png", 
                                 iconWidth = 10, 
                                 iconHeight = 10)

map <- leaflet::leaflet(width = 800, height = 800) %>% 
  leaflet::addTiles() %>% 
  leaflet::setView(lng = -101.05,
          lat = 11.68,
          zoom = 2)

map %>% leaflet::addMarkers(lng = MPCT_metadata_counts$long, 
                            lat = MPCT_metadata_counts$lat, 
                            icon = black.point, 
...
# load_prism(MPCT_metadata_counts, "./data", "/Volumes/DATA/PRISM/LT81_800M/")
...
coordsPoints <- sp::SpatialPoints(coords, proj4string=CRS("+proj=longlat +datum=NAD83 +no_defs +ellps=GRS80 +towgs84=0,0,0"))
...
data_long <- data.table::melt(climate.points,
                  # sample.id variables - all the variables to keep but not split apart.
                  id.vars="sample.id") %>% 
  tidyr::separate(col = "variable", 
                  into = c("garbage1", "variable", "garbage2", "garbage3", "garbage4", "yearmonth"), 
                  sep = "_") %>% 
  dplyr::select(-dplyr::starts_with("garbage")) %>% 
  tidyr::separate(col = "yearmonth", into = c("year", "month"), sep = 4) 
...
calc_gdd_monthly <- function(temp, t.base, t.cap=NULL, multiplier=1, to_fahrenheit=T, to_round=F){
  ...
  if(!is.null(t.cap)){
    ...
    temp_long <- dplyr::left_join(tmin, tmax, by = c("sample.id", "year", "month")) %>% 
