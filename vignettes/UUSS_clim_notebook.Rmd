...
UUSS_sites <- subset(UUSS_sites, !(UUSS_sites$site.id==1717 | UUSS_sites$site.id==3608 | UUSS_sites$site.id==3611 | UUSS_sites$site.id==15106 | UUSS_sites$site.id==15298 | UUSS_sites$site.id==15368))
...
fossil_sites <- unique(UUSS_sites$site.id)

# Subset the climate data to just Temperature. 
Temperature <- Clim_noNAs[,14:25]

# Create a list that will be used to reorder the temperature reconstruction columns, so that months are in order.
columnOrder <- c("site.id", "site.name", "lat", "long", "age", "MAT_tjan", "MAT_tfeb", "MAT_tmar", "MAT_tapr", "MAT_tmay", "MAT_tjun", "MAT_tjul", "MAT_taug", "MAT_tsep", "MAT_toct", "MAT_tnov", "MAT_tdec")

# Now, we carry out the reconstruction for all fossil pollen sites in the UUSS. Each site will have two output files, one will be the reconstructed temperatures and the other will have the associated error.

# First, to save some time, check to see if the sites have already been reconstructed, if so then this chunk will not run. If you would like to re-run the reconstructions, then simply delete the contents of the UUSS directory, then it will run. 
if (length(dir(path = "/Volumes/VILLAGE/SKOPEII/MAT/WORKING/paleomat/output/UUSS/", all.files=FALSE)) != 0) {
  
  
  # Check to see if all files from the reconstructed sites are in the UUSS output folder. 
  UUSS.files <- list.files(path = "/Volumes/VILLAGE/SKOPEII/MAT/WORKING/paleomat/output/UUSS/")
  UUSS.files <- unique(as.numeric(sapply(strsplit(UUSS.files, "_"), "[[", 1)))
  UUSS.files.missing <- setdiff(unique(UUSS_sites$site.id), UUSS.files)
  
  stop("The UUSS Reconstruction is already completed. If you would like to re-run the reconstructions, then delete all files from the UUSS directory.")
  
  } else {

  for (i in 1:length(unique(UUSS_sites$site.id))) {
    
    # Filter to one site to do the reconstruction.
    site <- dplyr::filter(UUSS_sites, site.id == fossil_sites[i])
    # site <- site[,-(1:8)] %>% 
    #   mutate_all(funs(ifelse(is.na(.), 0, .)))
    ...
    # Create an empty dataframe, so that the 12 months of data can be in one dataframe for the site.
    get_clim <- data.frame(matrix(ncol = 9, nrow = 0))
    
    # Name the columns for the new empty dataframe.
    x <- c("site.id", "site.name", "lat", "long", "age", "reconst", "err", "model", "climate")
    colnames(get_clim) <- x
    
    # The significance test currently has an error. Data types are the same, and there are the same number of variables. It is the same as running one site before. Will have to spend more time on this.
    # mat_reconst_sig <- run_tf2(pollen = MPCT_counts_noNAs, fossil = site[,9:ncol(site)], 
    #              climate = Temperature[,1:12],
    #              func = mat, col = 2, k = 10)
    
    # Need another loop to do the reconstruction for all 12 months for a site, which goes into one dataframe, get_clim.
    for (j in 1:ncol(Temperature)) {
      ...
      mat_reconst <- predict(rioja::MAT(y = MPCT_counts_noNAs, 
                                      ...
                                      lean = FALSE),
                           newdata = site[,9:ncol(site), drop=F],
                           sse = TRUE)
      
      # Put the reconstructed data into a dataframe.
      get_clim <-  rbind(get_clim, (data.frame(site.id = site$site.id,
                                               ...
                                               model   = rep("MAT", 
                                                             each = nrow(site)),
                                               climate = colnames(Temperature[j]))))
    }
    
      # write.csv(
      #   file = paste0("/Volumes/VILLAGE/SKOPEII/MAT/WORKING/paleomat/output/UUSS/", fossil_sites[i], sep = "", "_mat_reconst_sig.csv")
      # )
    
      # Output the reconstructed data into a csv file. 
      write.csv(get_clim %>% 
              ...
              row.names = FALSE,
              file = paste0("/Volumes/VILLAGE/SKOPEII/MAT/WORKING/paleomat/output/UUSS/", fossil_sites[i], sep = "", "_clim_reconst.csv"))
  
      # Output the error of the reconstructed data into a csv file.
      write.csv(get_clim %>% 
              ...
              dplyr::select(columnOrder),
              row.names = FALSE, 
            file = paste0("/Volumes/VILLAGE/SKOPEII/MAT/WORKING/paleomat/output/UUSS/", fossil_sites[i], sep = "", "_clim_err.csv"))
...
for (i in 1:length(fossil_sites)) {
  ...
  FS_reconst <- read.csv(paste0("/Volumes/VILLAGE/SKOPEII/MAT/WORKING/paleomat/output/UUSS/", fossil_sites[i], sep = "", "_clim_reconst.csv"))
  FS_err <- read.csv(paste0("/Volumes/VILLAGE/SKOPEII/MAT/WORKING/paleomat/output/UUSS/", fossil_sites[i], sep = "", "_clim_err.csv"))
  ...
  for (j in 1:12) {
    ...
    ggsave(month_plot, file = paste0(fossil_sites[i], "_", monthPlots[j], sep = "", "_plot.tiff"), path = "/Volumes/VILLAGE/SKOPEII/MAT/WORKING/paleomat/output/UUSS/monthly_plots/")
    ...
    htmlwidgets::saveWidget(month_plot_ggplotly,
                            paste0("/Volumes/VILLAGE/SKOPEII/MAT/WORKING/paleomat/output/UUSS/plotly_htmls/",
