...
UUSS_sites <- UUSS_sites[ , c(names(UUSS_sites)[1:8], sort(names(UUSS_sites)[9:ncol(UUSS_sites)]))]
...
#   site <- dplyr::filter(UUSS_sites, site.id == fossil_sites[1])
#   site <- site[,-(1:8)] %>%
#     mutate_all(funs(ifelse(is.na(.), 0, .)))
...
# 
# 
# mat_reconst_sig <- purrr::map(1:length(UUSS_sites$site.id),
#                               .f = function(k){
...
```{r summary_reconst, fig.width=8, fig.height=9, echo=FALSE}
...
UUSS_Puebloperiods <- UUSS_reconst %>% 
     dplyr::group_by(period=cut(age, breaks=c(265, 365, 465, 565, 665, 750, 805, 915, 1060, 1160, 1250, 1350, 1450, 1550, 1650, 1750, 1850, 2000))) %>%
     dplyr::summarise_at(c("reconst", "err"), funs(mean, n())) %>%
     dplyr::arrange(as.numeric(period)) %>% 
     mutate(date_midpoint = c(1635, 1535, 1435, 1335, 1242.5, 1173.5, 1090, 962.5, 840, 745, 650, 550, 450, 350, 250, 150, 50, 1845)) %>% 
     select(-ends_with("err_n"))

UUSS_Puebloperiods$period <- c("1585-1685 AD", "1485-1585 AD", "1385-1485 AD", "1285-1385 AD", "PIII Exploit", "PIII Explore", "PII Exploit", "PII Explore", "PI Exploit", "PI Explore", "BMIII Exploit", "BMIII Explore","400-500 AD", "300-400 AD","200-300 AD", "100-200 AD", "1-100 AD", "1685-2005 AD")
...
UUSS_Puebloperiods_July$period <- c("1585-1685 AD", "1485-1585 AD", "1385-1485 AD", "1285-1385 AD", "PIII Exploit", "PIII Explore", "PII Exploit", "PII Explore", "PI Exploit", "PI Explore", "BMIII Exploit", "BMIII Explore","400-500 AD", "300-400 AD","200-300 AD", "100-200 AD", "1-100 AD", "1685-2005 AD")
...
UUSS_Puebloperiods_January$period <- c("1585-1685 AD", "1485-1585 AD", "1385-1485 AD", "1285-1385 AD", "PIII Exploit", "PIII Explore", "PII Exploit", "PII Explore", "PI Exploit", "PI Explore", "BMIII Exploit", "BMIII Explore","400-500 AD", "300-400 AD","200-300 AD", "100-200 AD", "1-100 AD", "1685-2005 AD")
...
UUSS_Puebloperiods_plot <- ggplot(data=(dplyr::filter(UUSS_Puebloperiods[5:12,])), aes(x=date_midpoint, y=reconst_mean, color = reconst_mean)) +
      geom_line() +
  geom_ribbon(aes(ymin = reconst_mean - err_mean,
    ...
    # htmlwidgets::saveWidget(UUSS_Puebloperiods_plot_ggplotly, paste0("./output/UUSS/",
    #                               sep = "", "UUSS_Puebloperiods_plotly.html"))
