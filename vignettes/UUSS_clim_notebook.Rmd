...
colnames(NACT2)[1] <- "dataset"

# Switch site name and depth columns. Then, need to rename those two headings.
NACT2[ , c(2,3)] <- NACT2[ , c(3,2)]
colnames(NACT2)[c(2, 3)] <- c("site.name", "depth")

# Change dataset column data type from character to integer.
NACT2$dataset <- as.integer(NACT2$dataset)

# First, additional pollen taxa from the modern surface samples should be added, since we need to have the same column headings before adding the core tops data to the rest of the modern pollen data.
NACT3 <- NACT2 %>% 
...
#Last, re-order the column names to that of the modern pollen data.
NACT3 <- NACT3[,colnames(MP_metadata_counts)]

MP_metadata_counts_CTs <- rbind(MP_metadata_counts, NACT3)
...
states <- readOGR("/Volumes/VILLAGE/SKOPEII/MAT/WORKING/paleomat/data/statep010", layer='statep010')
...
EldoraFen <- get_download(FourCorners[[9]])
