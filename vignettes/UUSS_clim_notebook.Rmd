...
# Get Fossil Pollen locational site.id numbers.
NAfossilNames <- NAfossil_metadata_counts[,c("site.id","long","lat")] %>% 
  distinct(site.id, .keep_all = TRUE)

#Extract longitude and latitude data.
lonsFossil <- unlist(lapply(all,FUN=function(x){NAfossilNames$long}))
latsFossil <- unlist(lapply(all,FUN=function(x){NAfossilNames$lat}))

# Bind longitude and latitude and convert to SpatialPoints.
coordsFossil <- cbind(LONGITUDE=lonsFossil,LATITUDE=latsFossil)
coordsPointsFossil <- SpatialPoints(coordsFossil, proj4string=CRS("+proj=longlat +datum=NAD83 +no_defs +ellps=GRS80 +towgs84=0,0,0"))

studyArea <- polygon_from_extent(extent.FourCorners,"+proj=longlat +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +no_defs")
UUSS_sites2 <- raster::crop(coordsPointsFossil,rgeos::gUnaryUnion(spTransform(studyArea,CRS(projection(coordsPointsFossil))))) %>% 
  as.data.frame() %>% 
  set_colnames(c("long", "lat"))

# Subset fossil dataset.
UUSS_sites3 <- subset(NAfossil_metadata_counts, long > extent.FourCorners@xmin & long < extent.FourCorners@xmax)
UUSS_sites3 <- subset(UUSS_sites3, lat >= extent.FourCorners@ymin & lat <= extent.FourCorners@ymax)
...
# Floor the minimums, ceiling the maximums for the Four Corner States.
extent.FourCorners@xmin <- floor(extent.FourCorners@xmin)
extent.FourCorners@ymin <- floor(extent.FourCorners@ymin)
extent.FourCorners@xmax <- ceiling(extent.FourCorners@xmax)
extent.FourCorners@ymax <- ceiling(extent.FourCorners@ymax)

# Transform
UUSS_sites2 <- sp::spTransform(states, sp::CRS("+proj=longlat +datum=NAD83 +no_defs +ellps=GRS80 +towgs84=0,0,0"))
