...
# Download the publications for sites that have core tops. 
# CT_pubDate <- NAfossil_datasets %>% 
#   get_publication()
...
# First, merge the metadata with the publication year, using the site.id to match, just to double check.
CT_metadata_pub <- dplyr::left_join(CT_metadata,
                                    CT_pubDate,
                                    by  = "site.id")

# Then, bind the metadata_pub to the modern pollen counts.
# KB: Changed to left_join, which is safer in case row order changes
CT_metadata_counts <- dplyr::left_join(CT_metadata_pub,
                                       CT_counts,
                                       by  = "site.id") %>%
  dplyr::arrange(site.id)
