...
# Create one list of site.ids for each core top and surface sample sites. 
CT_noSites <- CT_metadata_counts$site.id
MP_noSites <- MP_metadata_counts$site.id
MPCT_noSites <- sort(c(CT_noSites, MP_noSites))

length(((duplicated(MPCT_noSites)) == TRUE))

if (!(length(unique(MPCT_noSites)) == length(MPCT_noSites))) {
stop()
} else {
  MPCT_metadata_counts <- dplyr::left_join(CT_metadata_counts,
                                       MP_metadata_counts,
                                       by  = "site.id") %>%
  dplyr::arrange(site.id)
}
