...
# Load and cleanup modern data (i.e., modern pollen, climate, and locations).

## Modern Pollen Data from Neotoma

```{r load_modernPollen, echo = FALSE, message=FALSE, warning=FALSE}
...
```{r load_modernPollenFromFossil, echo = FALSE, message=FALSE, warning=FALSE}
...
NAfossil_datasets <- neotoma::get_dataset(gpid = NAID, datasettype='pollen') %>% 
                        get_chroncontrol() %>% 
                        lapply("[", 1) %>% 
                        unlist(recursive=F)

# Get a list of the site IDs that are identifying the chron.control dataframes. Here, we get the unique site ID, then add it to the chronology data frames. 
NAfd_chronList <- lapply(names(NAfossil_datasets), function(x) x) %>% unlist() %>% as.data.frame()
...
coreTops <- NAfossil_datasets[[1]] %>% cbind(site.ID="site.ID") %>% dplyr::slice(0)

for(i in 1:length(NAfossil_datasets))
{
  tmpFrame <- cbind(NAfossil_datasets[[i]], site.ID=as.character(NAfd_chronList[i]))
...
```{r WA_sigTesting_init, cache=TRUE, results='asis', echo=FALSE, warning=FALSE}
...
```{r MAT_sigTesting_init, cache=TRUE, results='asis', echo=FALSE, warning=FALSE}
...
This returns `r length(FourCorners)` sites. The `neotoma` package provides plotting capabilities, but `leaflet` allows for more interactive plotting.
...
EldoraFen <- get_download(FourCorners[[12]])
...
fossil_data <- fossil_data %>% 
  bind_rows(as.data.frame(Pollen[1,]))
