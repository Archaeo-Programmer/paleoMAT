...
if (!file.exists("./output/UUSS_reconst_temp_5analog.csv")) {
  
  mat_models_temp_5analog <- purrr::map(1:12,
                              .f = function(j){
  ...
  UUSS_reconst_5analog <- mat_models_temp_5analog %>%
 ...
 write.csv(UUSS_reconst_5analog, "./output/UUSS_reconst_temp_5analog.csv", row.names = FALSE)
...
if (!file.exists("./output/UUSS_reconst_GDD_5analog.csv")) {
  
  mat_models_gdd_5analog <- purrr::map(1:12,
                              .f = function(j){
                                rioja::MAT(y = MPCT_counts_noNAs, 
                                           x = GDD_noNAs[,j], 
                                           k = 5, 
  ...
  UUSS_reconst_GDD_5analog <- mat_models_gdd_5analog %>%
  purrr::map_dfr(.id = "climate",
                 .f = function(the.model){
                   # Create the MAT reconstruction.
                   mat_reconst <- predict(the.model,
 ...
 write.csv(UUSS_reconst_GDD_5analog, "./output/UUSS_reconst_GDD_5analog.csv", row.names = FALSE)
...
UUSS_reconst_GDD_5analog_total <- UUSS_reconst_GDD_5analog %>% 
...
UUSS_reconst_GDD_5analog_meta <- UUSS_reconst_GDD_5analog %>% 
  dplyr::filter(climate == 5) %>%
  dplyr::select(-c(reconst, err, climate))

UUSS_GDD_5analog <- left_join(UUSS_reconst_GDD_5analog_meta, UUSS_reconst_GDD_5analog_total, by = c("dataset.id", "sample.id", "depth"))
...
UUSS_reconst_5analog <- read.csv("./output/UUSS_reconst_temp_5analog.csv")
...
UUSS_reconst_5analog <- UUSS_reconst_5analog %>% 
  dplyr::mutate(date = BPtoBCAD(UUSS_reconst_5analog$age))

UUSS_GDD_5analog <- UUSS_GDD_5analog %>% 
  dplyr::mutate(date = BPtoBCAD(UUSS_GDD_5analog$age))
...
UUSS_100yrAvgs <- UUSS_reconst_5analog %>% 
...
UUSS_100yrAvgs_5analog_GS <- UUSS_reconst_5analog %>%
     dplyr::filter(climate >= 5 & climate <= 9) %>% 
...
UUSS_100yrAvgs_5analog_GS_BP <- UUSS_reconst_5analog %>%
...
UUSS_100yrAvgs_5analog_July <- UUSS_reconst_5analog %>% 
...
UUSS_100yrAvgs_GDD_5analog <- UUSS_GDD_5analog %>% 
     dplyr::group_by(period=cut(date, breaks=seq(-100,2100,100))) %>% 
...
temp_UUSS_100yrAvgs_5analog_GS <- UUSS_100yrAvgs_5analog_GS
temp_UUSS_100yrAvgs_5analog_GS[2,7] <- 0
temp_UUSS_100yrAvgs_5analog_GS[21,7] <- 2000

UUSS_100yrAvgs_5analog_GS_plot <- ggplot(data=(dplyr::filter(temp_UUSS_100yrAvgs_5analog_GS[2:21, ])), aes(x=midP, y=reconst_mean, label = reconst_n, label2 = reconst_unique_sites)) + 
      ...
      scale_y_continuous(breaks=seq(9,18,1), limits = c(9, 18)) +
      theme_bw() + 
    ...
    ggsave(UUSS_100yrAvgs_5analog_GS_plot, file = "UUSS_100yrAvgs_5analog_GS_plot.tiff", path = "./output/UUSS/analog_7/", units="in", width=21, height=13, dpi=600, compression = 'lzw')
    
    # Create html files that have the interactive plots from plotly.
    UUSS_100yrAvgs_5analog_GS_plot_ggplotly <- ggplotly(UUSS_100yrAvgs_5analog_GS_plot, originalData = TRUE)
    f<-"./output/UUSS/plotly_htmls/UUSS_100yrAvgs_5analog_GS_plot_ggplotly.html"
     htmlwidgets::saveWidget(UUSS_100yrAvgs_5analog_GS_plot_ggplotly, file.path(normalizePath(dirname(f)),basename(f)))

     
temp_UUSS_100yrAvgs_5analog_July <- UUSS_100yrAvgs_5analog_July
temp_UUSS_100yrAvgs_5analog_July[2,7] <- 0
temp_UUSS_100yrAvgs_5analog_July[21,7] <- 2000

# Plot the warmest temperature month, July.
UUSS_100yrAvgs_5analog_July_plot <- ggplot(data=(dplyr::filter(temp_UUSS_100yrAvgs_5analog_July[2:21, ])), aes(x=midP, y=reconst_mean, label = reconst_n, label2 = reconst_unique_sites)) +
      #scale_color_gradient(low="blue", high="red") +
      geom_ribbon(aes(x=midP, ymin = reconst_mean - err_mean,
                      ymax = reconst_mean + err_mean,
                      alpha = 0.2),
    ...
    ggsave(UUSS_100yrAvgs_5analog_July_plot, file = "UUSS_100yrAvgs_5analog_July_plot.tiff", path = "./output/UUSS/analog_7/", units="in", width=21, height=13, dpi=600, compression = 'lzw')
    
    # Create html files that have the interactive plots from plotly.
    UUSS_100yrAvgs_5analog_July_plot_ggplotly <- ggplotly(UUSS_100yrAvgs_5analog_July_plot, originalData = TRUE)
    f<-"./output/UUSS/plotly_htmls/UUSS_100yrAvgs_5analog_July_plot_ggplotly.html"
     htmlwidgets::saveWidget(UUSS_100yrAvgs_5analog_July_plot_ggplotly, file.path(normalizePath(dirname(f)),basename(f)))
...
# Plot GDD
UUSS_100yrAvgs_GDD_plot <- ggplot(data=(dplyr::filter(UUSS_100yrAvgs_GDD)), aes(x=midP, y=reconst_sum_mean, color = reconst_sum_mean, label = reconst_sum_n, label2 = reconst_sum_unique_sites)) +
  geom_ribbon(aes(x=midP, ymin = reconst_sum_mean - err_mean_mean,
      ...
      scale_x_continuous(breaks=seq(-100,2100,100), minor_breaks = seq(-50, 2050, 200)) +
      scale_y_continuous(breaks = seq(500,1400,50)) +
      theme_bw() + 
      theme(panel.border = element_blank(), panel.grid.major = element_blank(), axis.text=element_text(size=22, colour = "black", family = "Helvetica"), axis.title.y = element_text(size = 28, family = "Helvetica", margin = margin(t = 10, r = 20, b = 10, l = 10)), axis.title.x = element_text(size = 28, family = "Helvetica", margin = margin(t = 20, r = 10, b = 10, l = 10)),
      panel.grid.minor = element_line(colour = "light grey"), axis.line = element_line(colour = "black"), legend.text = element_text(size = 22, family = "Helvetica"), legend.title = element_text(size = 22, family = "Helvetica"))
...
GDD_fit_5analog <- lm(reconst_sum_mean ~ elev_mean, data = UUSS_100yrAvgs_GDD_5analog)
tiff(file = "./Figures/GDD_elev_regression_5analog.tiff", height = 12, width = 15, units = 'in', 
     compression = "lzw", res = 600)
visreg(GDD_fit_5analog, xlab = "Elevation (m)", ylab = "Growing Degree Days", points=list(cex=.8))

dev.off()

summary(GDD_fit_5analog)

# Graph the residuals through time.
UUSS_100yrAvgs_GDD_5analog <- UUSS_100yrAvgs_GDD_5analog %>% 
  dplyr::mutate(residuals = GDD_fit_5analog$residuals)

# Plot GDD
temp_UUSS_100yrAvgs_GDD_5analog <- UUSS_100yrAvgs_GDD_5analog
temp_UUSS_100yrAvgs_GDD_5analog[2,7] <- 0
temp_UUSS_100yrAvgs_GDD_5analog[21,7] <- 2000

UUSS_100yrAvgs_GDD_5analog_Residuals_plot <- ggplot(data=(dplyr::filter(temp_UUSS_100yrAvgs_GDD_5analog[2:21, ])), aes(x=midP, y=residuals)) +
      geom_rect(aes(xmin=0,xmax=2000,ymin=-210,ymax=0,fill="blue"),alpha=0.009) +
      geom_rect(aes(xmin=0,xmax=2000,ymin=0,ymax=210,fill="red"),alpha=0.009) +
      scale_fill_identity() +
      geom_ribbon(aes(x=midP, ymin = residuals - sigma(GDD_fit),
                      ymax = residuals + sigma(GDD_fit),
                      alpha = 0.2),
                  fill = "grey",
                  colour="dark grey", 
                  show.legend = F) +
    ...
    # Save the plot for one month.
    ggsave(UUSS_100yrAvgs_GDD_5analog_Residuals_plot, file = "UUSS_100yrAvgs_GDD_5analog_Residuals_plot.tiff", path = "./output/UUSS/analog_7/", units="in", width=22, height=13, dpi=600, compression = 'lzw')
    
     UUSS_100yrAvgs_GDD_5analog_Residuals_plot_ggplotly <- ggplotly(UUSS_100yrAvgs_GDD_5analog_Residuals_plot)
     f<-"./output/UUSS/plotly_htmls/UUSS_100yrAvgs_GDD_5analog_Residuals_plot_ggplotly.html"
     htmlwidgets::saveWidget(UUSS_100yrAvgs_GDD_5analog_Residuals_plot_ggplotly, file.path(normalizePath(dirname(f)),basename(f)))
...
UUSS_reconst_5analog$climate <- as.integer(UUSS_reconst_5analog$climate)
FPdata_avgs$climate <- as.integer(FPdata_avgs$climate)

FPdata_avgs_monthly <- dplyr::left_join(UUSS_reconst_5analog, FPdata_avgs, by= c("dataset.id", "climate"))
...
UUSS_100yrAvgs_GDD_5analog_anomaly <- FPdata_avgs_monthly %>% 
     dplyr::filter(climate >= 5 & climate <= 9) %>% 
     dplyr::group_by(period=cut(date, breaks=seq(-100,2100,100))) %>% 
...
# Plot the anomaly for GS.
temp_UUSS_100yrAvgs_GDD_5analog_anomaly <- UUSS_100yrAvgs_GDD_5analog_anomaly
temp_UUSS_100yrAvgs_GDD_5analog_anomaly[2,7] <- 1
temp_UUSS_100yrAvgs_GDD_5analog_anomaly[21,7] <- 2000

UUSS_100yrAvgs_GDD_5analog_anomaly_plot <- ggplot(data=(dplyr::filter(temp_UUSS_100yrAvgs_GDD_5analog_anomaly[2:21, ])), aes(x=midP, y=anomaly_mean, label = reconst_n, label2 = reconst_unique_sites)) +
    ...
    # Save the plot for GS.
    ggsave(UUSS_100yrAvgs_GDD_5analog_anomaly_plot, file = "UUSS_100yrAvgs_GDD_5analog_anomaly_plot.tiff", path = "./output/UUSS/analog_7/", units="in", width=13.3, height=3, dpi=200, compression = 'lzw')
    
    # Create html files that have the interactive plots from plotly.
    UUSS_100yrAvgs_GDD_5analog_anomaly_plot_ggplotly <- ggplotly(UUSS_100yrAvgs_GDD_5analog_anomaly_plot, originalData = TRUE)
    f<-"./output/UUSS/plotly_htmls/UUSS_100yrAvgs_GDD_5analog_anomaly_plot_ggplotly.html"
     htmlwidgets::saveWidget(UUSS_100yrAvgs_GDD_5analog_anomaly_plot_ggplotly, file.path(normalizePath(dirname(f)),basename(f)))
