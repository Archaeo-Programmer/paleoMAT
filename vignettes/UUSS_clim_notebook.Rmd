...
if (!file.exists("./output/UUSS_reconst_temp.csv")) {
 ...
 write.csv(UUSS_reconst, "./output/UUSS_reconst_temp.csv", row.names = FALSE)
...
UUSS_GDD <- left_join(UUSS_reconst_GDD_meta, UUSS_reconst_GDD_total, by = c("dataset.id", "sample.id", "depth"))
...
temp_UUSS_100yrAvgs_GS <- UUSS_100yrAvgs_GS
temp_UUSS_100yrAvgs_GS[2,7] <- 0
temp_UUSS_100yrAvgs_GS[21,7] <- 2000

UUSS_100yrAvgs_GS_plot <- ggplot(data=(dplyr::filter(temp_UUSS_100yrAvgs_GS[2:21, ])), aes(x=midP, y=reconst_mean, label = reconst_n, label2 = reconst_unique_sites)) + 
    ...
    ggsave(UUSS_100yrAvgs_GS_plot, file = "UUSS_100yrAvgs_GS_plot.tiff", path = "./output/UUSS/months/", units="in", width=21, height=13, dpi=600, compression = 'lzw')
    
    # Create html files that have the interactive plots from plotly.
    UUSS_100yrAvgs_GS_plot_ggplotly <- ggplotly(UUSS_100yrAvgs_GS_plot, originalData = TRUE)
    f<-"./output/UUSS/plotly_htmls/UUSS_100yrAvgs_GS_plot_ggplotly.html"
...
UUSS_100yrAvgs_July_plot <- ggplot(data=(dplyr::filter(UUSS_100yrAvgs_July)), aes(x=midP, y=reconst_mean, label = reconst_n, label2 = reconst_unique_sites)) +
      #scale_color_gradient(low="blue", high="red") +
      geom_ribbon(aes(x=midP, ymin = reconst_mean - err_mean,
                      ymax = reconst_mean + err_mean,
                      alpha = 0.2),
    ...
    ggsave(UUSS_100yrAvgs_July_plot, file = "UUSS_100yrAvgs_July_plot.tiff", path = "./output/UUSS/months/", units="in", width=21, height=13, dpi=600, compression = 'lzw')
    
    # Create html files that have the interactive plots from plotly.
    UUSS_100yrAvgs_July_plot_ggplotly <- ggplotly(UUSS_100yrAvgs_July_plot, originalData = TRUE)
...
# Plot GDD
temp_UUSS_100yrAvgs_GDD <- UUSS_100yrAvgs_GDD
temp_UUSS_100yrAvgs_GDD[2,7] <- 0
temp_UUSS_100yrAvgs_GDD[21,7] <- 2000

UUSS_100yrAvgs_GDD_Residuals_plot <- ggplot(data=(dplyr::filter(temp_UUSS_100yrAvgs_GDD[2:21, ])), aes(x=midP, y=residuals)) +
    ...
    ggsave(UUSS_100yrAvgs_GDD_Residuals_plot, file = "UUSS_100yrAvgs_GDD_Residuals_plot.tiff", path = "./output/UUSS/", units="in", width=22, height=13, dpi=600, compression = 'lzw')
     ...
     UUSS_100yrAvgs_GDD_Residuals_plot_ggplotly <- ggplotly(UUSS_100yrAvgs_GDD_Residuals_plot)
     f<-"./output/UUSS/plotly_htmls/UUSS_100yrAvgs_GDD_Residuals_plot_ggplotly.html"
     htmlwidgets::saveWidget(UUSS_100yrAvgs_GDD_Residuals_plot_ggplotly, file.path(normalizePath(dirname(f)),basename(f)))
...
pdf(file='./Figures/UUSS_delta_1200-1300.pdf', width=fig.width, height=fig.height, bg="white", pointsize=8, version="1.7")
par(bg='white',fg='black',col.lab='black', col.main='black', col.axis='black', font=2, lend='round',ljoin='round')
...
raster::plot(temprast14, col=colors, useRaster=T, legend=F, xlab="", ylab="", add=T)
plot(temprast14, legend.only=TRUE, col=colors, legend.shrink=0.25, legend.args=list(text='Temperature (°C)', side=4, font=2, line=2.5, cex=0.6))
