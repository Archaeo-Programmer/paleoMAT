...
# First, to save some time, check to see if the sites have already been reconstructed, if so then this chunk will not run. The dir is listed to find out how many .csv files are currently in the output folder (if any). Then, on the right side, we use the length of the fossil sites times two (since 2 files are created for each site, 1 reconstruction and 1 error). If you would like to re-run the reconstructions, then simply delete the contents of the UUSS directory, then it will run. ((length(fossil_sites))*2)
if (length(dir(path = "./output/UUSS/", all.files=FALSE, pattern = "*.csv")) == ((length(fossil_sites))*2)) {
  
  # Check to see if all files from the reconstructed sites are in the UUSS output folder. 
  # UUSS.files <- list.files(path = "./output/UUSS/", pattern = "*.csv")
  # UUSS.files <- unique(as.numeric(sapply(strsplit(UUSS.files, "_"), "[[", 1)))
  UUSS.files.missing <- setdiff(UUSS.files, unique(UUSS_sites$site.id))
  
  print(paste0(UUSS.files.missing))
  
  stop(print(paste0("The UUSS Reconstruction is already completed. If you would like to re-run the reconstructions, then delete all files from the UUSS directory. However, the following sites have not yet been reconstructed: ", UUSS.files.missing)))
...
stop()
  ...
  for (i in 1:length(unique(UUSS_sites$site.id))) {
    ...
    for (j in 1:ncol(Temperature)) {
      ...
      # Output the significance test of the fossil pollen site and the calibrated dataset.
      # write.csv(
      #   file = paste0("./output/UUSS/", fossil_sites[i], sep = "", "_mat_reconst_sig.csv")
      # )
      ...
      write.csv(get_clim %>% 
              ...
              row.names = FALSE,
              file = paste0("./output/UUSS/", fossil_sites[i], sep = "", "_clim_reconst.csv"))
      ...
      write.csv(get_clim %>% 
              ...
              dplyr::select(columnOrder),
              row.names = FALSE, 
            file = paste0("./output/UUSS/", fossil_sites[i], sep = "", "_clim_err.csv"))
