...
Pub_Year <- data.frame(site.id=numeric(),
                      pub_year=numeric())
...
tmpFrame <- data.frame(site.id=numeric(),
                      pub_year=numeric())
...
MP_metadata_pub <- merge(MP_metadata, MP_pubDate, by  = "site.id")

# Then, bind the metadata_pub to the modern pollen counts.
MP_metadata_counts <- bind_cols(MP_metadata_pub,
                                MP_counts)
...
#### Obtaining a fossil pollen record for all sites in UUSS dataset
...
for (i in 1:length(fossil_sites)) {
  nam <- paste("mat_reconst_", fossil_sites[[1]], sep = "")
  dat <- run_tf2(pollen = MP_counts_noNAs, fossil = fossil_3060, 
       climate = Clim_noNAs[,13:24],
       func = mat, col = 2, k = 10)
  assign(nam, dat)
}
