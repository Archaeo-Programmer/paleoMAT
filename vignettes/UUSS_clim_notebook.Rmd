...
# Extract the one site that is along the border.
mexico_metadata_counts <- mexico_metadata_counts %>% 
...
CT_metadata_counts <- dplyr::left_join(coreTops,
  ...
  rbind.fill(mexico_metadata_counts) %>% 
...
MPCT_metadata_counts <- rbind.fill(MP_metadata_counts, CT_metadata_counts) %>%
  dplyr::arrange(dataset.id) # %>%
...
MPCT_metadata <- MPCT_metadata_counts[,1:10]
MPCT_counts <- MPCT_metadata_counts[,c(11:ncol(MPCT_metadata_counts))] %>% 
  mutate_all(funs(ifelse(is.na(.), 0, .)))

MPCT_metadata_counts <- cbind(MPCT_metadata, MPCT_counts)

# Sort the taxa to be in alphabetical order.
MPCT_metadata_counts <- MPCT_metadata_counts[,c(names(MPCT_metadata_counts)[1:10], sort(names(MPCT_metadata_counts)[11:ncol(MPCT_metadata_counts)]))]
...
climate.points <- dplyr::rename(climate.points, dataset.id = ID)
...
MPCT_counts_noNAs <- MPCT_metadata_counts_noNAs[,-(1:10)] %>% 
  mutate_all(funs(ifelse(is.na(.), 0, .)))
...
# Plot all monthly temperatures lumped together. 
...
UUSS_100yrAvgs_GDD_edited <- UUSS_100yrAvgs_GDD
...
UUSS_100yrAvgs_GDD_plot <- ggplot(data=(dplyr::filter(UUSS_100yrAvgs_GDD)), aes(x=midP, y=reconst_sum_mean, color = reconst_sum_mean, label = reconst_sum_n, label2 = reconst_sum_unique_sites)) +
  geom_ribbon(aes(x=midP, ymin = reconst_sum_mean - err_mean_mean,
      ...
      scale_x_continuous(breaks=seq(-100,2100,100), minor_breaks = seq(-50, 2050, 200)) +
      scale_y_continuous(breaks = seq(900,1400,50)) +
      theme_bw() + 
      theme(panel.border = element_blank(), panel.grid.major = element_blank(), axis.text=element_text(size=22, colour = "black", family = "Helvetica"), axis.title.y = element_text(size = 28, family = "Helvetica", margin = margin(t = 10, r = 20, b = 10, l = 10)), axis.title.x = element_text(size = 28, family = "Helvetica", margin = margin(t = 20, r = 10, b = 10, l = 10)),
      panel.grid.minor = element_line(colour = "light grey"), axis.line = element_line(colour = "black"), legend.text = element_text(size = 22, family = "Helvetica"), legend.title = element_text(size = 22, family = "Helvetica"))
