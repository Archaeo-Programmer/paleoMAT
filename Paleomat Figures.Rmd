...
ggplot() +
  ...
  scale_color_manual(values = c("darkgrey" = "darkgrey", "red2" = "red2"),
...
ggsave("LF_reconstruction_extent.png", dpi = 600)
...
temp_anom_standard <- site.preds.unlisted.anom %>% 
  group_by(date) %>% 
  dplyr::summarise(temp = mean(anom)) %>% 
  dplyr::mutate(update = (temp - mean(temp)) / sd(temp)) %>% 
  dplyr::mutate(Series = "This study") %>% 
  dplyr::select(year = date, update, Series)
