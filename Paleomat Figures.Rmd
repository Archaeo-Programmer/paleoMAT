...
```{r fig3, fig.cap="Locations of fossil pollen samples used in this study and extent of the Upper United States Southwest (Bocinsky and Kohler 2014) over the mean July temperature for 1961 to 1990 from PRISM at 800-m resolution.", fig.width=10.5, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all'}
...
# temp_anom_standard <- temp_anom %>% 
#   dplyr::mutate(update = (anom - mean(temp_anom$anom)) / sd(temp_anom$anom))

# # Moberg LF and paleomat
#   ggplot() +
#   geom_line(data = temp_anom_standard, aes(x = year, y = update), colour = "red", size = 1.0) +
#   geom_line(data = Moberg_standard, aes(x = year, y = update)) +
#   xlab("Year BC/AD") +
#   ylab("July Temperature Anomaly (°C)") +
#   theme_bw()
...
```{r fig7, fig.cap="Just vep ii area reconstructions", fig.width=10.5, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all'}
...
# Calculate the anomaly across time for cmv (to produce line graph).
...
# Calculate the anomaly across time for nrg (to produce line graph).
...
anom_nrg %>%
  ggplot() +
  geom_line(aes(x = year, y = anom), colour = "red", size = 1.0) +
  geom_line(data = anom_cmv, aes(x = year, y = anom), colour = "black", size = 1.0) +
  xlab("Year BC/AD") +
  ylab("July Temperature Anomaly (°C)") +
  scale_x_continuous(breaks = seq(-1000, 1800, 200),
                     minor_breaks = seq((-1000 + 100), (1800 - 100), 200)) +
