...
```{r fig1, fig.cap="Modern samples in North America.", echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all'}
...
```{r fig3, fig.cap= "Temperature for the North Atlantic using Greenland ice core data (Alley 2000) for 20,000 BP to present.", fig.width=10.5, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all'}
...
```{r fig4, fig.cap="Low-frequency component of Moberg et al. (2005) multi-proxy reconstruction for Northern Hemisphere for 1817-25 BP.", fig.width=10.5, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all'}
...
```{r fig5, fig.cap="Low-frequency temperature reconstruction time series for this study.", fig.width=10.5, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='hide',fig.keep='all'}

Threebest <- read_csv("~/Dropbox/WSU/SKOPEII/model/Anom_All3Steps_90sign_withBootstrapErrors_4analogs.csv") %>%
  filter(date >= -3000)
...
# Extracting the fit values

# p <- Threebest %>%
#   filter(date >= -3000) %>%
#   qplot(date,anom_bootstrap,data=.) +
#   stat_smooth(aes(outfit=fit<<-..y..), span = 0.05, n = 5000)
# 
# anom_loess3 <- ggplot_build(p)$data[[2]]
# 
# ggplot(anom_loess) +
#   geom_line(aes(x, outfit))
...
#viau_paleomat <- bind_rows(temp_anom_standard, viau_standard)


viau_paleomat_kaufman$Series <- factor(viau_paleomat_kaufman$Series, c("Viau et al. 2006", "Kaufman et al. 2020", "This study"))

ggplot(iris, aes(row.names(iris), Sepal.Width, color = Species, group = Species)) +
...
# Moberg_paleomat_dff <- left_join(temp_anom_standard %>% dplyr::select(-Series), Moberg_standard %>% dplyr::select(-Series), by = "year") %>% 
#   mutate(diff = anom.x - anom.y) %>% 
#   dplyr::select(-c(anom.x, anom.y))
...
Moberg_paleomat %>% 
  ...
  geom_hline(yintercept = 0, color = "darkgrey", lty = 2, lwd = 1.25) +
  # geom_vline(aes(xintercept = 872, color = "mean"), linetype="twodash", lwd = 0.5) +
  # geom_vline(xintercept = c(1002, 1144, 1225, 1286, 1447, 1582), color = "red", linetype="twodash", lwd = 0.5) +
  geom_line(size = 2.0) +
...
#Moberg paleomat diff
ggplot(data = Moberg_paleomat_dff, aes(x = year, y = diff)) +
  geom_line(size = 2, color = "#A60F2D") +
  xlab("Years AD") +
  ylab("Temperature Anomaly (°C)") +
  # scale_x_continuous(breaks=seq(0,2000,200),
...
ggsave("~/Dropbox/WSU/SKOPEII/model/Final_Figures/Figure7_diff.png", dpi = 600)
...
fossil.locs2 <-
  ...
  dplyr::select("Site Name" = site.name, "Site ID" = site.id, "Dataset ID" = dataset.id, "No. of Samples" = n, "Elevation (m)" = elev, "Longitude" = long, "Latitude" = lat)
...
ggplot(data = rlen_results_filtered, aes(x = age, y = Squared_res_lengths)) +
  ...
  scale_y_continuous(limits = c(0, 260),
                     ...
                     guide = "axis_minor") +
  theme_bw() +
  geom_hline(yintercept = quantile(rlen$train, probs = 0.95), color = "red", lty = 1, lwd=0.5) +
  theme(
    axis.ticks.length=unit(0.125,"inch"),
    ggh4x.axis.ticks.length.minor = rel(0.5),
    panel.border = element_blank(),
...
tbl <- cbind(swap.mat2$standard$rmsep[1:20], swap.mat2$standard$r.squared[1:20],
             swap.mat2$standard$avg.bias[1:20], swap.mat2$standard$max.bias[1:20])
