...
summary_table_final <-
  ...
  left_join(
    .,
    fossil_final_west_metadata_post5500BP %>% ungroup %>% dplyr::select(site.name, site.id, dataset.id) %>% distinct() %>% st_drop_geometry(),
    by = "dataset.id"
  ) %>% 
...
names(summary_table_final) <- c("Site Name", "Site ID", "Dataset ID", "Proportion explained variance", "Random TF p", 
                                "Anomaly (°C)", "Anomaly Bootstrapped (°C)", "Mean boostrap RMSEP (°C)", "Mean boostrap s1")

write.csv(summary_table_final, "./vignettes/data/summary_table_final.csv", row.names = FALSE)
