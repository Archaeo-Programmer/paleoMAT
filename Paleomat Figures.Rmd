...
Threebest %>%
  ...
  geom_hline(yintercept = 0,
             linetype = 'dashed',
             color = c('blue')) +
  geom_smooth(method = "loess", span = 0.15, aes(x = date), size = 2) +
  scale_x_continuous(limits = c(-3000, 2100),
                     breaks = seq(-3000, 2000, 100),
                     labels = head(as.vector(rbind(seq(-3000, 2100, 500), "", "", "", "")), -4)) +
  scale_y_continuous(limits = c(-8, 8),
                     breaks = seq(-8, 8, 1),
                     labels = head(as.vector(rbind(seq(-8, 8, 2), "")), - 1)) +
  xlab("Years BC/AD") +
  ylab("Temperature Anomaly (°C)") +
  theme(
    panel.border = element_blank(),
